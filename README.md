...
A mod starter for loading, mutating, and reporting configuration-style state without burying the example in framework noise.
...
Built jars are written to `build/libs/hytale-mod-template-config-1.1.0.jar`, with matching sources and javadoc jars next to it.

## Commands
- `/hdconfigmodstatus`: Shows runtime status for ConfigModPlugin.
- `/hdconfigmoddemo`: Runs a demo action for ConfigModPlugin.
...
2. Copy `build/libs/hytale-mod-template-config-1.1.0.jar` into your server `mods/` directory.
