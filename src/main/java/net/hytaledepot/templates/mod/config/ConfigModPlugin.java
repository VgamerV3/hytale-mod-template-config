...
public final class ConfigModPlugin extends JavaPlugin {
  ...
  private final ConfigModTemplate service = new ConfigModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-config-mod-worker");
  ...
  public ConfigModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new ConfigModStatusCommand());
    getCommandRegistry().registerCommand(new ConfigModDemoCommand());
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[ConfigMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[ConfigMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class ConfigModStatusCommand extends CommandBase {
    private ConfigModStatusCommand() {
      super("hdconfigmodstatus", "Shows runtime status for ConfigModPlugin.");
  ...
  private final class ConfigModDemoCommand extends CommandBase {
    private ConfigModDemoCommand() {
      super("hdconfigmoddemo", "Runs a demo action for ConfigModPlugin.");
    setAllowsExtraArguments(true);
      this.setPermissionGroup(GameMode.Adventure);
    }
